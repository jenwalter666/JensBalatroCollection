...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float len = 1.8*(0.7 - length(to_center) + 0.12 * sin(bulged.x));
    
    vec4 extra = Texel(texture, texture_coords - to_center * 25. * len/ image_details);
	
    return dissolve_mask(color * 0. + 1. * vec4(extra.xyz, color.a), texture_coords, uv);
}
