...
extern PRECISION vec2 sharpened;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float N = 5. + abs(sin(sharpened.x)) * 0.3;

	vec4 result = vec4(0);

	for (int i = -1; i <= 1; i += 2) {
		vec4 color_at = Texel(texture, texture_coords + vec2(i / image_details.x, 0));

		result += color_at;
	}
	for (int j = -1; j <= 1; j += 2) {
		vec4 color_at = Texel(texture, texture_coords + vec2(0, j / image_details.y));

		result += color_at;
	}

	return dissolve_mask(vec4(pixel.rgb * N - result.rgb, pixel.a), texture_coords, uv);
}
