...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float r = (original_pixel.r * .393) + (original_pixel.g *.769) + (original_pixel.b * .189);
	float g = (original_pixel.r * .349) + (original_pixel.g *.686) + (original_pixel.b * .168);
	float b = (original_pixel.r * .272) + (original_pixel.g *.534) + (original_pixel.b * .131);
    
	vec2 uv = (((texture_coords)*(image_details)) - texture_details.xy*texture_details.ba)/texture_details.ba;

	vec4 processed_pixel = vec4(r, g, b, original_pixel.a);

	float intensity = 0.90 + 0.09 * sin(sepia.x * 2);
    
	return dissolve_mask(intensity * processed_pixel + (1. - intensity) * original_pixel, texture_coords, uv);
}
