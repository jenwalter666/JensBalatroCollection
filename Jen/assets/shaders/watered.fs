...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float weight = 0.07 + sin(watered.x) * 0.04;

	for (int i = -2; i < 2; i++) {
		for (int j = -2; j < 2; j++) {
			pixel -= weight * (vec4(0.9) - Texel(texture, texture_coords + vec2(i / image_details.x, j / image_details.y)));
		}
	}
