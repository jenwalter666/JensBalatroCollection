...
// Card rotation
extern PRECISION vec2 missingtexture;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	hsl.z = 1. - floor(hsl.z * 10.) / 10.;

	float value = step(0.,
        1. *sin((uv.x) * 300. * hsl.z)  * sin((uv.y) * 300. * hsl.z)
    	);

	vec4 final_pixel = vec4(value, 0., value, pixel.a);

	if (final_pixel.a > 9999) final_pixel.a = missingtexture.x;
