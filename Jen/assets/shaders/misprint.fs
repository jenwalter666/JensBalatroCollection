...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float shiftX = 0.3 * sprite_width + 0.0000000000001 * misprint.x;

    if (uv.x > 0.80) {
        texture_coords.x -=  (uv.x-0.8) * sprite_width;
    }

    if (uv.x > 0.27 && uv.x < 0.35) {
        texture_coords.x -= (uv.x - 0.50) * sprite_width;
        
        if (uv.y > 0.02 && uv.y < 0.98) {
            texture_coords.y -= (uv.y - floor(uv.y * 14.)/14.) * sprite_height;
        } else {
            return vec4(0);
        }
    }

    float newX = texture_coords.x - shiftX;
    if (newX < min_x) {
        newX += sprite_width;
    } else if (newX > max_x) {
        newX -= sprite_width;
