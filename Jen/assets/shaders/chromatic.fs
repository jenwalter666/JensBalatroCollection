...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float r  = Texel(texture, texture_coords + (1.5 + sin(time * 3) + 0.5 * cos(1.5* chromatic.x)) *  vec2(1.9) * normalize_and_amp).r;
    float g = Texel(texture, texture_coords).g;
    float b = Texel(texture, texture_coords - (1.5 + sin(time * 3) + 0.5 * cos(1.5 * chromatic.y)) * vec2(1.9)  * normalize_and_amp).b;
    
    
	return dissolve_mask(vec4(r, g, b, color.a) * colour, texture_coords, uv);
}
